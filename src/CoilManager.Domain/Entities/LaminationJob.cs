...
public sealed class LaminationJob : SoftDeletableEntity
{
    ...
    public decimal TotalConsumedWeight { get; private set; } public decimal TotalScrapWeight { get; private set; }
    ...
    public void Complete(int good, int rejected, decimal consumed, decimal scrap, string? remarks, string actor, DateTimeOffset now) { if(Status!=LaminationJobStatus.Allocated)throw new InvalidOperationException("Only an Allocated job can be completed."); if(good<0||rejected<0||consumed<0||scrap<0)throw new InvalidOperationException("Completion values cannot be negative."); TotalGoodPieces=good; TotalRejectedPieces=rejected; TotalConsumedWeight=consumed; TotalScrapWeight=scrap; CompletionRemarks=Clean(remarks); CompletedBy=actor; CompletedOn=now; Status=LaminationJobStatus.Completed; }
