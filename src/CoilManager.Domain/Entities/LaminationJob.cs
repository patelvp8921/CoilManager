...
public sealed class LaminationJob : SoftDeletableEntity
{
    ...
    public void Complete(int good, int rejected, decimal consumed, decimal scrap, decimal totalJobWeight, string? remarks, string actor, DateTimeOffset now) { if(Status!=LaminationJobStatus.Allocated)throw new InvalidOperationException("Only an Allocated job can be completed."); if(good<0||rejected<0||consumed<0||scrap<0||totalJobWeight<0)throw new InvalidOperationException("Completion values cannot be negative."); TotalGoodPieces=good; TotalRejectedPieces=rejected; TotalConsumedWeight=consumed; TotalScrapWeight=scrap; TotalJobWeight=totalJobWeight; CompletionRemarks=Clean(remarks); CompletedBy=actor; CompletedOn=now; Status=LaminationJobStatus.Completed; }
