    ...
    ICurrentUserService currentUser) : IWorkOrderService
{
    ...
    public async Task<WorkOrderDetailsDto> CreateAsync(CreateWorkOrderRequest request, CancellationToken cancellationToken = default)
    {
        ...
        await unitOfWork.ExecuteInTransactionAsync(async ct =>
        {
            ...
            try { workOrder = Build(number, request); }
    ...
    {
        Validate(request); WorkOrder wo = await FindAsync(id, cancellationToken); EnsureRowVersion(wo, request.RowVersion);
    ...
    public Task<WorkOrderDetailsDto> ReleaseAsync(Guid id, CancellationToken cancellationToken = default) => Transition(id, (x, a, t) => x.Release(a, t), cancellationToken);
    ...
    public async Task<WorkOrderMaterialAllocationDto> AllocateAsync(Guid id, CreateMaterialAllocationRequest request, CancellationToken cancellationToken = default)
    {
        ...
        await unitOfWork.ExecuteInTransactionAsync(async ct =>
        {
            ...
            if (wo.Status is not (WorkOrderStatus.Released or WorkOrderStatus.InProduction)) throw new BusinessRuleException("Material may only be allocated to a Released or In Production Work Order.");
            ...
            wo.AddAllocation(allocation); await unitOfWork.SaveChangesAsync(ct);
        }, cancellationToken);
    ...
    public async Task<IReadOnlyList<AvailableCoilDto>> GetAvailableMotherCoilsAsync(Guid id, string? search, CancellationToken cancellationToken = default) { _ = await FindAsync(id, cancellationToken); return await repository.GetAvailableMotherCoilsAsync(search, cancellationToken); }
    public async Task<IReadOnlyList<AvailableCoilDto>> GetAvailableSlitCoilsAsync(Guid id, string? search, CancellationToken cancellationToken = default) { _ = await FindAsync(id, cancellationToken); return await repository.GetAvailableSlitCoilsAsync(search, cancellationToken); }
    ...
    private async Task<WorkOrderDetailsDto> MapAsync(WorkOrder x, CancellationToken token)
    {
        ...
        return new(x.Id, x.WorkOrderNumber, x.WorkOrderType, x.ProductType, x.CustomerName, x.SalesOrderReference, x.WorkOrderDate,
    ...
    private static WorkOrder Build(string number, CreateWorkOrderRequest r) => new(number, r.WorkOrderType, r.ProductType, r.CustomerName,
        r.SalesOrderReference, r.WorkOrderDate, r.RequiredDate, r.Priority, r.GradeId, r.Thickness, r.Category, r.CoreLossPerKg,
