...
public sealed class LaminationJobsController(ILaminationJobService service,IWebHostEnvironment environment) : BaseApiController
{
 ...
 [HttpPut("{id:guid}/allocations/{allocationId:guid}")] public async Task<ActionResult<ApiResponse<LaminationJobMaterialAllocationDto>>> UpdateAllocation(Guid id,Guid allocationId,UpdateLaminationAllocationRequest r,CancellationToken t)=>Success(await service.UpdateAllocationAsync(id,allocationId,r,t),"Allocation updated.");
 ...
 [HttpPost("{id:guid}/skip-allocation")] public async Task<ActionResult<ApiResponse<LaminationJobDetailsDto>>> SkipAllocation(Guid id,CancellationToken t)=>Success(await service.SkipAllocationAsync(id,t),"Material allocation skipped; manual issue will be recorded by production.");
