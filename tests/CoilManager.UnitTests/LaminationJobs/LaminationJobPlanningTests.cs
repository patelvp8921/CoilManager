...
public sealed class LaminationJobPlanningTests
{
 ...
 [Fact]public void Released_job_cannot_complete_directly(){var j=Job();j.AddStep(Step());j.Release("planner",DateTimeOffset.UtcNow);Assert.Throws<InvalidOperationException>(()=>j.Complete(10,0,10,0,null,"planner",DateTimeOffset.UtcNow));}
 [Fact]public void Allocated_job_can_complete_only_once(){var j=Job();j.AddStep(Step());j.Release("planner",DateTimeOffset.UtcNow);j.ConfirmAllocation("planner",DateTimeOffset.UtcNow);j.Complete(10,0,10,0,null,"planner",DateTimeOffset.UtcNow);Assert.Equal(LaminationJobStatus.Completed,j.Status);Assert.Throws<InvalidOperationException>(()=>j.Complete(10,0,10,0,null,"planner",DateTimeOffset.UtcNow));}
 [Fact]public void Completed_job_cannot_be_cancelled(){var j=Job();j.AddStep(Step());j.Release("planner",DateTimeOffset.UtcNow);j.ConfirmAllocation("planner",DateTimeOffset.UtcNow);j.Complete(10,0,10,0,null,"planner",DateTimeOffset.UtcNow);Assert.Throws<InvalidOperationException>(()=>j.Cancel("planner",DateTimeOffset.UtcNow));}
